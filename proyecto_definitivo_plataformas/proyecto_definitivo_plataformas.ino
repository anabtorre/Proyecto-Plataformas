...
/* Bibliotecas incluidas para el sensor de temperatura y humedad, y para el Bluetooth*/
#include "DHT.h"
...
int lecturaHigrom;     /*variable para guardar la lectura del Higrometro*/
int higrometro = A0;   /*ubicación en la placa de Arduino, entrada del Sensor Higrometro*/
 
/*Pines a los que estan conectados en el arduino los reles*/
...
int ledCalentador= 13;               /*LED que debe encenderse si calentador esta encendido*/  
...
void setup() {
 ...
 pinMode (relayAbanico,OUTPUT);    /*Configurar relayAbanico como salida*/
 pinMode (relayCalentador,OUTPUT); /*Configurar relayCalentador como salida*/ 
...
void loop() {    
    ...
    delay(5000);
    controlTempHum(tempMax, tempMin, humMin, humMax);
...
/* NaN (not a number) --> si t (valor medido de temperatura) o h (valor medido de humedad), no es un numero se imprime que hubo un fallo.*/
 ...
 /*OJO: los reles en LOW encienden, en HIGH apagan*/
      ...
      if (tempLeida>tempMax && humLeida>humMax){
        ...
        digitalWrite(relayAbanico,HIGH); //Si la humedad sobrepasa el valor maximo, se ENCIENDE el abanico
